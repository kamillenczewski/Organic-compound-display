...
ALKANE_GROUPS_AND_COAL_AMOUNTS = {"metylo":1, "etylo":2, "propylo":3, "butylo":4, "pentylo":5}
...
def string_numbers_to_numbers(str_numbers):
    return [int(str_number) for str_number in str_numbers]

def strings_to_integer_lists(strings: list[str]):
    return [string_numbers_to_numbers(split_by_comma(string)) for string in strings]
...
def add_group_in_direction(iterator: CoalChainIterator, coal_index, group, direction):
    iterator.move_to_coal_index(coal_index)

    match(direction):
        case "up":
            add_group_up(iterator, group)
        case "down":
            add_group_down(iterator, group)
        case "right":
            add_group_right(iterator, group)
        case "left":
            add_group_left(iterator, group)

def add_element_in_direction(iterator, coal_index, group, direction):
    symbol = get_symbol(group)
    add_group_in_direction(iterator, coal_index, symbol, direction)

def add_alkane_group_in_direction(iterator: CoalChainIterator, coal_index, group, direction):
    iterator.move_to_coal_index(coal_index)

    match(direction):
        case "up":
            add_alkane_group_up(iterator, group)
        case "down":
            add_alkane_group_down(iterator, group)

def add_hydroxyl_group(iterator, coal_index, direction):
    add_group_in_direction(iterator, coal_index, "OH", direction)

SPECIAL_GROUPS_SUFFIXES_AND_METHOD_TO_ADD_THEM = {"ol":add_hydroxyl_group}

class MainGroupInterpreter1:
    def __init__(self, main_group_items) -> None:
        self.main_group_items = main_group_items
        self.validate_items()

    def validate_items(self):
        if len(self.main_group_items) < 3:
            raise ValueError("The main group doesn't contain sufficient information about the compound!")

    def get_coal_amount(self):
        return alkane_prefix_to_coal_amount(self.get_alkane_prefix())

    def get_alkane_prefix(self):
        return self.main_group_items[0]
    ...
    def has_special_groups(self):
        return len(self.main_group_items) > 3

    def get_special_group_add_methods(self):
        pass
...
def interprate_compound_name(compound_name):
    ...
    for coal_index, groups in enumerate(coal_indexed_groups):
        ...
        for group in groups:
            ...
            if is_element(group):
                add_element_in_direction(coal_chain_iterator, coal_index, group, direction)
            elif group.startswith("ol"):
                add_hydroxyl_group(coal_chain_iterator, coal_index, direction)
            else:
                add_alkane_group_in_direction(coal_chain_iterator, coal_index, group, direction)

    return matrix

COMPOUNDS = [
    "4-bromo-1,2-dichloro-7-etylo-3,3,7-trimetylo-5,5-dipropylonon-1-en-1-ol",
    ...
    "1,2-dietylo-4,4-dibromo-5-chloro-3,3,6,6-tetrametylooktan",
]
